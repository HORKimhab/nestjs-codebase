...
export const Auth = (...args: (AuthType | UserRole)[]) => {
  const authTypes = args.filter(item => Object.values(AuthType).includes(item as AuthType)) as AuthType[];
  const roles = args.filter(item => Object.values(UserRole).includes(item as UserRole)) as UserRole[];
